...
func CardbusinessAggregatepayB2cOnlineConsumepurchase() {
	...
	bizContentMap := map[string]string{
		"mer_id":           config.AppConfig.String("onlineMerId"),   //商户编号
		...
		"mer_prtcl_no":     config.AppConfig.String("merPrtclNo"),    //收单产品协议编号
		"orig_date_time":   "2021-01-29T10:13:00",                    //交易日期时间，格式为yyyy-MM-dd'T'HH:mm:ss,注意：测试环境交易日期需要咨询工行
		"decive_info":      "xxxxx09090erwsfws",                      //设备号，随便填写
		...
		"mer_url":          config.AppConfig.String("notifyUrl"),     //异步通知商户URL，端口必须为443或80
		"shop_appid":       config.AppConfig.String("wxAppId"),       //商户在微信开放平台注册的APPID，支付方式为微信时不能为空,如：微信appID,微信小程序appID
		"icbc_appid":       config.AppConfig.String("appId"),         //商户在工行API平台的APPID
		"open_id":          config.AppConfig.String("wxAppletAppId"), //第三方用户标识，商户在微信公众号内或微信小程序内接入时必送，即access_type为7或9时，上送用户在商户APPID下的唯一标识；商户通过支付宝生活号接入时不送
		"mer_acct":         config.AppConfig.String("notifyUrl"),     //商户账号，商户入账账号，只能交易时指定
