...
func CardbusinessZfbh5UiH5consumption() {
	...
	bizContentMap := map[string]string{
		"mer_id":          config.AppConfig.String("onlineMerId"), // 必填，商户线下档案编号,特约商户12位，特约部门15位
		"mer_prtcl_no":    config.AppConfig.String("merPrtclNo"),  // 必填，收单产品协议编号
		"order_id":        "x900990dfs0",                          // 必填，商户订单号，只能是数字、大小写字母，且在同一个商户号下唯一
		"order_date_time": "2021-02-04T12:00:00",                  // 必填，交易日期时间，格式为yyyy-MM-dd'T'HH:mm:ss,注意：测试环境交易日期需要咨询工行
		"amount":          "12",                                   //单位分
		"cur_type":        lib.FEE_TYPE_CNY,                       // 必填，交易币种，目前工行只支持使用人民币（001）支付
		"body":            "H5在线支付",                               // 必填，商品描述
		"notify_url":      config.AppConfig.String("notifyUrl"),   // 必填，异步通知商户URL，端口必须为443或80
		"icbc_appid":      config.AppConfig.String("appId"),       // 必填，工行API平台的APPID
