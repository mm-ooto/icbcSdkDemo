...
func TestZfbH5Pay(t *testing.T) {
	...
	bizContentMap := map[string]string{
		...
		"order_id":        "x9009900",         // 必填，商户订单号，只能是数字、大小写字母，且在同一个商户号下唯一
		...
		"body":            "H5在线支付",           // 必填，商品描述
		"notify_url":      "*******",                 // 必填，异步通知商户URL，端口必须为443或80
	...
	bizContentBytes, err := json.Marshal(bizContentMap)
	if err != nil {
		t.Log(err.Error())
		return
	}
	action := "/ui/cardbussiness/zfbh5/ui/h5consumption/V1"
